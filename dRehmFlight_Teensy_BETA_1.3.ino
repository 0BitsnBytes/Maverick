...
void setup() {
  ...
  m1.attach(m1Pin, 900, 2100);
  m2.attach(m2Pin, 900, 2100);
...
void loop() {
  ...
  int switchPos = getSwitchPos(channel_6_pwm);
  targetAlpha = switchPosToAlpha(switchPos);
...
void controlMixer() {
   ...
   *ALPHA IS THE VARIABLE FOR LINEAR INTERPOLATION AND TRANSITIONING INTO FORWARD FLIGHT, ALPHA = 1 IS HOVER, ALPHA = 0 IS FORWARD FLIGHT
   */

  //Bicopter mixing
  //0.5 is centered servo, 0.0 is zero throttle if connecting to ESC for conventional PWM, 1.0 is max throttle
  m1_command_scaled = thro_des + roll_PID - alpha * yaw_PID;                                   //R MOTOR, motors control roll during hover and yaw in forward flight
  s1_command_scaled = (alpha * range) + offset - alpha * pitch_PID * 3 + alpha * yaw_PID + (1-alpha)*roll_PID*4; /*R TRANS, 2/3 the range of the servo (servo is 135 deg, trans is 90 deg), 
                       //                                                                                offset by 1/6 of the range to have a centered start, 4 is the sensitivity*/
  Serial.println(alpha);

  m2_command_scaled = thro_des - roll_PID - alpha * yaw_PID;                                         //L MOTOR, motors control roll during hover and yaw in forward flight
  s2_command_scaled = 1 - ((alpha * range) + offset - alpha * pitch_PID * 3 - alpha * yaw_PID + (1-alpha)*roll_PID*4); /*L TRANS, 2/3 the range of the servo (servo is 135 deg, trans is 90 deg), 
                       //                                                                                offset by 1/6 of the range to have a centered start, 4 is the sensitivity*/

  s3_command_scaled = 0.37 + 3 * pitch_PID;  //ELEV, 0.37 is the offset and 3 is the sensitivity
  s4_command_scaled = 0.37 + 3 * pitch_PID;  //ELEV, 0.37 is the offset and 3 is the sensitivity
  s5_command_scaled = 0;
}
...
int getSwitchPos(int pwm) {
  if (pwm < 1300) return 0;
  if (pwm < 1700) return 1;
  return 2;
}

float switchPosToAlpha(int pos) {
  switch (pos) {
    case 0: return 0.0f;
    case 1: return 0.5f;
    case 2: return 1.0f;
...
void updateAlpha(float ALPHA_STEP) {
...
void commandMotors() {
  // Write microseconds to ESCs / motors (standard PWM)
